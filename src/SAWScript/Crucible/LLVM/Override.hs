...
ppLLVMVal cc val = do
  ...
  liftIO $ Crucible.ppLLVMValWithGlobals sym (Crucible.memImplSymbolMap mem) val
...
ppArgs sym cc cs (Crucible.RegMap args) = do
  ...
  case Crucible.lookupGlobal (Crucible.llvmMemVar (ccLLVMContext cc)) (cc^.ccLLVMGlobals) of
    ...
    Just mem -> do
      traverse (Crucible.ppLLVMValWithGlobals sym (Crucible.memImplSymbolMap mem) . fst) =<<
